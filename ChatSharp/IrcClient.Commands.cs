...
namespace ChatSharp
{
    public partial class IrcClient
    {
        ...
        /// <summary>
        /// Properly sends a multiline message to one or more destinations (channels or users).
        /// </summary>
        public void SendMultilineMessage(string message, bool skipEmpty, params string[] destinations)
        {
            var lines = message.Replace("\r", "").Split('\n');
            foreach(var line in lines)
                if(line.Length > 0 || !skipEmpty)
                    SendMessage(line, destinations);
        }
        ...
        /// <summary>
        /// Properly sends a multiline CTCP action to one or more destinations (channels or users).
        /// </summary>
        public void SendMultilineAction(string message, bool skipEmpty, params string[] destinations) {
            var lines = message.Replace("\r", "").Split('\n');
            foreach(var line in lines)
                if(line.Length > 0 || !skipEmpty)
                    SendAction(line, destinations);
        }
        ...
        /// <summary>
        /// Properly sends a multiline NOTICE to one or more destinations (channels or users).
        /// </summary>
        public void SendMultilineNotice(string message, bool skipEmpty, params string[] destinations) {
            var lines = message.Replace("\r", "").Split('\n');
            foreach(var line in lines)
                if(line.Length > 0 || !skipEmpty)
                    SendNotice(line, destinations);
        }
